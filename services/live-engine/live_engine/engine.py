"""The Live Trading Engine — the same pipeline as paper, but orders are placed
through the Broker Gateway (a real broker adapter) instead of the paper broker.

    Strategy → OMS.submit → Risk.check
             → approved? OMS.route + BrokerGateway.place_order
                          + OMS.record_fill + Risk.record_fill
             → rejected? OMS.reject

In dry-run the gateway records orders locally (no money). Going live is purely a
config change (real credentials + BROKER_DRY_RUN=false); this loop is unchanged.
After the run it reconciles OMS positions against the broker's.
"""

from __future__ import annotations

from collections.abc import Iterable
from dataclasses import dataclass, field
from datetime import datetime

from broker_gateway.gateway import BrokerGateway
from libs.common.market import Bar
from libs.common.models import CanonicalOrder, OrderStatus, OrderType, Side
from libs.common.strategy import Context, Strategy
from oms.book import OrderBook
from risk_engine.engine import RiskEngine


@dataclass
class LiveResult:
    broker: str
    starting_equity: float
    final_equity: float
    realized_pnl: float
    orders_submitted: int = 0
    orders_filled: int = 0
    orders_rejected: int = 0
    positions: dict[str, int] = field(default_factory=dict)
    reconciled: bool = True
    discrepancies: list[dict[str, object]] = field(default_factory=list)
    equity_curve: list[tuple[datetime, float]] = field(default_factory=list)

    @property
    def return_pct(self) -> float:
        if self.starting_equity == 0:
            return 0.0
        return (self.final_equity - self.starting_equity) / self.starting_equity * 100.0


class _LiveContext:
    def __init__(self, oms: OrderBook, symbol: str) -> None:
        self._oms = oms
        self._symbol = symbol
        self._pending: list[tuple[Side, int]] = []

    @property
    def position(self) -> int:
        return self._oms.position(self._symbol).quantity

    def buy(self, quantity: int) -> None:
        if quantity > 0:
            self._pending.append((Side.BUY, quantity))

    def sell(self, quantity: int) -> None:
        if quantity > 0:
            self._pending.append((Side.SELL, quantity))

    def take_pending(self) -> list[tuple[Side, int]]:
        pending, self._pending = self._pending, []
        return pending


class LiveTradingEngine:
    def __init__(
        self,
        risk: RiskEngine,
        oms: OrderBook,
        gateway: BrokerGateway,
        starting_equity: float = 1_000_000.0,
    ) -> None:
        self._risk = risk
        self._oms = oms
        self._gateway = gateway
        self._starting_equity = starting_equity

    def run(self, strategy: Strategy, bars: Iterable[Bar], symbol: str = "NIFTY") -> LiveResult:
        ctx = _LiveContext(self._oms, symbol)
        result = LiveResult(
            broker=self._gateway.broker_name,
            starting_equity=self._starting_equity,
            final_equity=self._starting_equity,
            realized_pnl=0.0,
        )
        cash = self._starting_equity
        strategy_name = type(strategy).__name__
        seq = 0
        strategy.on_start(ctx)

        for bar in bars:
            for side, qty in ctx.take_pending():
                seq += 1
                cash += self._execute(
                    strategy, ctx, result, symbol, strategy_name, seq, side, qty, bar
                )
            strategy.on_bar(ctx, bar)
            position = self._oms.position(symbol).quantity
            result.equity_curve.append((bar.timestamp, cash + position * bar.close))

        strategy.on_finish(ctx)
        result.final_equity = result.equity_curve[-1][1] if result.equity_curve else cash
        result.realized_pnl = self._oms.realized_pnl
        result.positions = {s: p.quantity for s, p in self._oms.positions().items()}

        # Reconcile our book against the broker's reported positions.
        result.discrepancies = self._gateway.reconcile(result.positions)
        result.reconciled = not result.discrepancies
        return result

    def _execute(
        self,
        strategy: Strategy,
        ctx: Context,
        result: LiveResult,
        symbol: str,
        strategy_name: str,
        seq: int,
        side: Side,
        qty: int,
        bar: Bar,
    ) -> float:
        # Route as a limit order at this bar's open so the (dry-run) broker has a price.
        order = CanonicalOrder(
            symbol=symbol,
            side=side,
            quantity=qty,
            order_type=OrderType.LIMIT,
            limit_price=bar.open,
        )
        managed = self._oms.submit(order, idempotency_key=f"{symbol}-{seq}")
        result.orders_submitted += 1

        decision = self._risk.check(
            order, bar.open, strategy=strategy_name, now=bar.timestamp.timestamp()
        )
        if not decision.approved:
            self._oms.reject(managed.id, "; ".join(decision.reasons))
            result.orders_rejected += 1
            strategy.on_order_update(ctx, managed.id, OrderStatus.REJECTED)
            return 0.0

        self._oms.mark_pending(managed.id)
        broker_result = self._gateway.place_order(order)  # real broker (dry-run safe)
        self._oms.record_fill(
            managed.id, broker_result.filled_quantity, broker_result.average_price
        )
        self._risk.record_fill(order, broker_result.average_price, broker_result.filled_quantity)
        result.orders_filled += 1
        strategy.on_order_update(ctx, managed.id, OrderStatus.FILLED)

        signed = qty if side is Side.BUY else -qty
        return -(signed * broker_result.average_price)
